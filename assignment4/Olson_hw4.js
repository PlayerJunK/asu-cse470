...
function init() {
  ...
  scene.lights.push(new Light(vec4(1.5,0,-1.5,1), vec3(1,1,1), 1))
  scene.lights.push(new Light(vec4(0,0,2,1), vec3(1,1,1), 2))
  ...
  cylinder.transform.translate(-1.25, 0, 0)
  ...
  pot.transform.translate(1.25, 0, 0)
...
function render() {
  ...
  scene.lights[2].position = vec4(-2.5 * Math.sin(frames / 100), 0.5, 1.5 * Math.cos(frames / 100), 1)
  scene.draw(gl)
